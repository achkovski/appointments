...
export const sendAppointmentConfirmationEmail = async (appointment) => {
  const { clientEmail, clientFirstName, appointmentDate, startTime, businessName } = appointment;
  ...
  if (transporter) {
    const info = await transporter.sendMail(mailOptions);
    console.log('✉️  Appointment confirmation email sent:', info.messageId);
    return info;
  } else {
    console.log('📧 [EMAIL] Appointment confirmation email (not sent)');
